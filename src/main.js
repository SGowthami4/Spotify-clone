...
const getDinnerPlaylist = () => fetchRequest(END_POINTS.DINNER_PLAYLIST);
...
function loadPlaylistCategories({ message: name, playlists: { items } }) {
  ...
  //   <h1 class="text-2xl font-semibold">Cooking& Dining</h1>
  //   <section
  ...
  for (let { description, images, id, name } of items) {
    ...
    mainContainer.className =
      "overflow-hidden group flex relative items-center justify-between gap-2 flex-col p-2 rounded-md cursor-pointer hover:bg-gray";
    mainContainer.innerHTML = `<img
              ...
              <button
                class="bg-spotify-green absolute right-6 bottom-6 grid place-items-center size-12 rounded-full transition-all delay-300 ease-linear opacity-0 group-hover:opacity-100 group-hover:bottom-12"
              >
                <svg
...
function loadPlaylistTracks({ tracks }) {
  ...
  for (let trackItem of tracks.items.filter((item) => item.track.preview_url)) {
    ...
    track.id = id;
    track.className =
      "cursor-pointer hover:bg-gray track p-1 grid grid-cols-[50px_1fr_1fr_40px] gap-4 items-center justify-items-start rounded-md";
    let image = album.images.find((img) => img.height === 64);
    let artistNames = Array.from(artists, (artist) => artist.name).join(", ");
    track.innerHTML = `
    <p class="relative w-full flex items-start justify-start justify-self-center">
        <span class="track-no">${trackNo++}</span>
    </p>
    <section class="grid grid-cols-[auto_1fr] place-items-center gap-2">
      <img class="size-10 rounded-sm" src=${image.url} alt=${name}/>
    <article class="flex flex-col justify-start items-start">
        <h2 class="line-clamp-1 song-title">${name}</h2>
        <p class="line-clamp-1 text-subdued text-xs">${artistNames}</p>
    </article>
    </section>
    ...
    playButton.className =
      "play w-full absolute right-4  text-lg invisible  material-symbols-outlined";
  ...
  saveItemLocalStorage("loadedTracks", loadedTracks);
}
...
async function fillContentForPlaylist(playlistId) {
  ...
  mainView.innerHTML = `
      <article
          id="cover-content"
          class="-m-2 grid grid-rows-[200px] grid-cols-[auto_1fr] gap-4 bg-gradient-to-b from-gray to-secondary p-4"
        >
          <img
            class="self-end object-contain rounded-md size-40"
            src="${images[0].url}"
            alt="${name} cover image"
          />
          <section class="flex flex-col gap-2">
            <h2 class="text-6xl font-bold" id="playlist-name">${name}</h2>
            <p id="playlist-description" class="text-sm text-subdued">
              ${description}
            </p>
            <ul
              id="playlist-details"
              class="flex items-center gap-4 text-subdued list-disc"
            >
              <li class="list-none">
                <section class="text-white flex gap-2 pr-6">
                  <img
                    class="size-6 object-contain"
                    src="/Primary_Logo_Green_CMYK.svg"
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const previousTrack = document.querySelector("#prev");
  ...
  function onProfileClick(event) {
    const nextElementsId = event.target.nextElementSibling.id;
    const menu = document.getElementById(`${nextElementsId}`);
  ...
  audio.addEventListener("play", () => {
    ...
    progressInterval = setInterval(() => {
      if (audio.paused) {
        ...
        songDurationCompleted.textContent = `0:${audio.currentTime <= 9 ? "0" + audio.currentTime.toFixed(0) : "" + audio.currentTime.toFixed(0)}`;
  ...
  previousTrack.addEventListener("click", playPrevTrack);

  // onPlaylistClick("37i9dQZF1DX0XUfTFmNBRM");
